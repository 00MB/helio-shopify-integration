...
// ÷*******************************************************
const metaMaskprotectRoute = (req, res, next) => {
    ...
    try {
      ...
      if (token) {
        Packages.jwt.verify(token, process.env.JWT_SECRET, async (err, data) => {
          ...
          // .select('-password')
          Modals.MetaMask.findById({ _id: data._id }).then(data => {
            if (data && data.token == token) {
              ...
              return res.status(401).json({
                success: false,
                msg: "MetaMask account not found",
                token: false
              })
            }
          })
...
module.exports = { metaMaskprotectRoute };
