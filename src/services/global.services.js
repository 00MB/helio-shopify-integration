...
module.exports = {
  JwtToken: (payLoad) =>
    packages.jwt.sign(payLoad, process.env.JWT_SECRET, { expiresIn: "90d" }),
  returnResponse: (res, status, error, msg, data) =>
    res.status(status).json({ error, msg, data }),
  // **********************************************************************
  verifyPassword: (encryptedPass, simplePassword) =>
    packages.bcrypt.compare(simplePassword, encryptedPass),
  createOtp: () => {
